  ...
  <div class="container-fluid">
    <div class="animated fadeIn">
      <div class="card">
        ...
        <div class="kotak">
          <b-button variant="success"><router-link to="/user/add">Add Data</router-link></b-button>
        </div>
...
import axios from 'axios';
  ...
  export default {
    ...
    methods: {
      deleteItem(row) {
        let app = this
        axios.delete(apiUrl() + '/users/' + this.deleteID + '?token=' + app.$session.get('token'), {
      ...
      getdata () {
        let app = this
        axios.get(apiUrl()+'/users?token=' + app.$session.get('token'), {
            'Access-Control-Allow-Origin': '*',
            'Content-Type': 'application/json'
        }).then(function (res) {
          app.items = res.data
