  ...
  <div class="container-fluid">
    <div class="animated fadeIn">
      <div class="card">
        ...
        <div class="kotak">
          <b-button variant="success"><router-link to="/agen/add">Add Data</router-link></b-button>
        </div>
  ...
  export default {
    data () {
      return {
        ...
        fields: [
          { key: 'id', label: 'No' },
          { key: 'nm_agen', label: "Nama Agen" },
          { key: 'pemilik', label: "Nama Pemilik" },
          { key: 'no_telp', label: "No Telepon" },
          { key: 'nm_kab', label: "Nama Kabupaten" },
          { key: 'nm_kec', label: "Nama Kecamatan" },
          { key: 'nm_desa', label: "Nama Desa" },
    ...
    methods: {
      deleteItem(row) {
        let app = this
        axios.delete(apiUrl() + '/agen' + this.deleteID + '?token=' + app.$session.get('token'), {
      ...
      editItem(row) {
        this.$router.push({ name: 'editagen', params: { id: row.id} })
      },
      getdata () {
        let app = this
        axios.get(apiUrl()+'/agen?token=' + app.$session.get('token'), {
            'Access-Control-Allow-Origin': '*',
            'Content-Type': 'application/json'
        }).then(function (res) {
          app.items = res.data
